  ...
  <div x-show="openModal" x-transition class="fixed left-0 top-0 w-full min-h-screen z-20 pt-20 bg-custom-black/20 flex flex-col justify-start items-center">
    <div class="w-3/4 md:w-2/6 bg-slate-600 rounded-md">
      <div class="bg-slate-800 flex flex-row justify-between">
        <p class="pl-2">Update Instansi</p>
      ...
      <div>
        <form wire:submit.prevent='updateInstansi' class="flex flex-col gap-2 m-4">
          ...
          <button @click="toogleModal()" class="w-fit self-center bg-orange-500 px-2 py-1 text-stone-800 hover:bg-orange-400 hover:text-stone-900 rounded-sm" type="submit">Update Instansi</button>
        </form>
      </div>
    </div>
  </div>
